...
from error.error import BanIPError,ExPandaError,ProxyError
...
COOKIE_DICK = {'igneous': '', 'ipb_member_id': '',
               'ipb_pass_hash': '9',
               'lv': ''}
...
EX_URL = 'https://exhentai.org/g/%s/%s/'
...
def getfav_rat(gid,token,IPandport):
    ...
    try:
        ex = requests.get(EX_URL % (gid, token), headers=ehheaders, cookies=EX_COOKIE, timeout=10, proxies =proxies)
