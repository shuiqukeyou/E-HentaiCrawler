...
from config import COOKIE_DICT,ABS_PROXY
...
def getfav_rat(gid,token,IPandport):
    ...
    try:
        ex = requests.get(EX_URL % (gid, token), headers=ehheaders, cookies=excookies, timeout=10, proxies =proxies)
    ...
    if 'Archive Download' not in html and 'IP' in html:
        ...
        for language in bsobj.find(text="Language:").parent.next_sibling:
            ...
            break
        data = {"favorited": f, 'ratings': r, 'elanguage': language, 'title_jpn': jtitle}
        return data
